...
static void ArrayIterator_setval(ArrayIteratorClass *this, ...)
{
    ...
    array_size = this->_array->_size;
    if (this->_idx > array_size)
        return;
    va_start(tmp, this);
    free(this->_array->_tab[this->_idx]);
    this->_array->_tab[this->_idx] = va_new(this->_array->_type, &tmp);
    va_end(tmp);
}
...
static void Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    for(size_t i = 0; i < this->_size; i++){
        va_copy(tmp, *args);
        this->_tab[i] = va_new((Class *)this, &tmp);
    }
}
...
static Object *Array_getitem(ArrayClass *this, ...)
{
    ...
    va_start(tmp_list, this);
    pos = va_arg(tmp_list, size_t);
    va_end(tmp_list);
    if (pos > this->_size)
        return NULL;
...
static void  Array_setitem(ArrayClass *this, ...)
{
    ...
    va_start(tmp, this);
    pos = va_arg(tmp, size_t);
    if (pos > this->_size)
        return;
    free(this->_tab[pos]);
    this->_tab[pos] = va_new(this->_type, &tmp);
    va_end(tmp);
}
